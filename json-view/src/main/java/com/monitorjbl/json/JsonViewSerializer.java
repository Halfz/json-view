...
public class JsonViewSerializer extends JsonSerializer<JsonView> {
     ...
     * Thus, when JSonView find a field of that type (DateTime), it will delegate the serialization to the serializer
     * specified.<br>
     * Example:<br>
     * <code>
    ...
    public <T> void registerCustomSerializer(Class<T> cls, JsonSerializer<T> forType) {
        ...
        if (cls == null) {
            ...
            throw new IllegalArgumentException("Class " + cls + " already has a serializer registered (" +
                    customSerializersMap.get(cls) + ")");
        }
    ...
    class JsonWriter {
        ...
        private JsonWriter(JsonGenerator jgen, JsonView result, Match currentMatch, SerializerProvider
                serializerProvider) {
            this.jgen = jgen;
            this.result = result;
        ...
        //internal use only to encapsulate what the current state was
        private JsonWriter(JsonGenerator jgen, JsonView result, Match currentMatch, String currentPath, Stack<String>
                path, AccessibleProperty referringField, SerializerProvider serializerProvider) {
            this.jgen = jgen;
            this.result = result;
            this.currentMatch = currentMatch;
        ...
        boolean writeList(Object obj) throws IOException {
            if (obj instanceof List || obj instanceof Set || obj.getClass().isArray()) {
                ...
                for (Object o : iter) {
                    new JsonWriter(jgen, result, currentMatch, currentPath, path, referringField, serializerProvider)
                            .write(null, o);
                }
                jgen.writeEndArray();
            } else {
                    ...
                    if (fieldAllowed(property, obj.getClass())) {
                        ...
                        if (fieldSerializer != null) {
                            ...
                            if (serializer != null) {
                                ...
                                new JsonWriter(jgen, result, currentMatch, currentPath, path, property,
        ...
        boolean valueAllowed(AccessibleProperty property, Object value, Class cls) {
            Include defaultInclude = serializerProvider.getConfig() == null ? Include.ALWAYS : serializerProvider
                    .getConfig().getSerializationInclusion();
            ...
            return value != null || (defaultInclude == Include.ALWAYS && jsonSerialize == null) || (jsonSerialize !=
                    null && jsonSerialize.include() == Inclusion.ALWAYS);
        }
...
//      });
        ...
        void write(String fieldName, Object value) throws IOException {
            ...
            if (!writePrimitive(value) && !writeSpecial(value) && !writeEnum(value) && !writeList(value) && !writeMap
                    (value)) {
                writeObject(value);
            }
        ...
        int containsMatchingPattern(Set<String> values, String pattern, boolean matchPrefix) {
            return memoizer.matches(values, pattern, matchPrefix, () -> {
                ...
                for (String val : values) {
                    ...
                    if (Pattern.compile(replaced).matcher(pattern).matches() || (matchPrefix && val.startsWith
                            (pattern + "."))) {
        ...
        boolean annotatedWithIgnore(AccessibleProperty f) {
            return memoizer.annotatedWithIgnore(f, () -> {
                ...
                JsonIgnoreProperties classIgnoreProperties = getAnnotation(f.declaringClass, JsonIgnoreProperties
                        .class);
                ...
                if (getAnnotation(f, JsonBackReference.class) != null && referringField != null) {
                    for (AccessibleProperty lastField : getAccessibleProperties(referringField.declaringClass)) {
                        JsonManagedReference fieldManagedReference = getAnnotation(lastField, JsonManagedReference
                                .class);
                ...
                return (jsonIgnore != null && jsonIgnore.value()) || (classIgnoreProperties != null && asList
                        (classIgnoreProperties.value()).contains(f.name)) || (fieldIgnoreProperties != null && asList
                        (fieldIgnoreProperties.value()).contains(f.name)) || backReferenced;
            });
        }
        ...
        private List<AccessibleProperty> getAccessibleProperties(Class cls) {
            return memoizer.accessibleProperty(cls, () -> {
                ...
                getDeclaredFields(cls).stream().map(f -> new AccessibleProperty(f.getName(), f.getAnnotations(), f))
                        .forEach(p -> accessibleProperties.put(p.name, p));
                getDeclaredMethods(cls).stream().filter(m -> (m.getName().startsWith("get") || m.getName().startsWith
                        ("is")) && !m.getReturnType().equals(Void.class) && m.getParameters().length == 0).map(m ->
                        new AccessibleProperty(getFieldNameFromGetter(m), m.getAnnotations(), m)).forEach(p -> {
                    ...
                    // TODO: Makes sure combined annotations are applied to the field when method visibility is
                    // disallowed
                ...
                return accessibleProperties.values().stream().filter(p -> visible.test(p.property)).collect
                        (Collectors.toList());
            });
        }
        ...
        private List<Method> getDeclaredMethods(Class cls) {
            ...
            while (!parents.isEmpty()) {
                ...
                if (c.getInterfaces() != null) {
                    Stream.of(c.getInterfaces()).forEach(parents::push);
                }
            }
        ...
        private Annotation[] getAnnotations(Class cls) {
            return memoizer.annotations(cls, cls::getAnnotations);
        }
